...
public class BoardFormValidator implements Validator , PasswordValidator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        if (!memberUtil.isLogin()) {
            ...
            if (!StringUtils.hasText(guestPw)) { // 1. 필수 여부
                errors.rejectValue("guestPw", "NotBlank");
            }
            if (StringUtils.hasText(guestPw) && guestPw.length() < 6) { // 2. 6자리 이상
                errors.rejectValue("guestPw", "Size");
            }
            if (StringUtils.hasText(guestPw) && (!alphaCheck(guestPw, true) || !numberCheck(guestPw))) { // 3. 알파벳 + 숫자
