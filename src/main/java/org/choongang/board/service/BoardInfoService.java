...
public class BoardInfoService {
    ...
    public ListData<BoardData> getList(String bid, BoardDataSearch search) {
        ...
        int limit = Utils.onlyPositiveNumber(search.getLimit(), board.getRowsPerPage()); // 설정값 가져온다.
        ...
        if (StringUtils.hasText(skey)) {
            ...
            if (sopt.equals("SUBJECT")) { // 제목
                ...
                orBuilder.or(boardData.poster.contains(skey))
                        ...
                        .or(boardData.member.username.contains(skey)); // 닉네임말고 회원명
                andBuilder.and(orBuilder);
            }
        }

        // 특정 사용자로 게시글 한정 : 마이페이지에서 활용 가능
        String userId = search.getUserId(); // 아이디 값으로 특정 회원게시글 가져오기, 마이페이지에서 활용
    ...
    private void addBoardData(BoardData boardData) {
        ...
        AbstractMember _member = boardData.getMember(); // null - 비회원, X null -> 회원
